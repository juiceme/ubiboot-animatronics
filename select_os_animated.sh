...
load_harmattan()
{
  BOOTKERNEL=$1
  menu_fadeout
  TMP_COMMAND_LINE=$(echo "$O_COMMAND_LINE" | sed -e "s/ init\=\/sbin\/preinit/init\=\/sbin\/preinit_harmattan/")
  F_COMMAND_LINE="\"$TMP_COMMAND_LINE\""
...
load_nitdroid()
{
  BOOTKERNEL=$1
  menu_fadeout
  TMP_COMMAND_LINE=$(echo "$O_COMMAND_LINE" | sed -e "s/ init\=\/sbin\/preinit/init\=\/sbin\/preinit_nitdroid/")
  F_COMMAND_LINE="\"$TMP_COMMAND_LINE\""
  eval "kexec -l --type=zImage --command-line=$F_COMMAND_LINE $BOOTKERNEL"
  if [ $? -eq 0 ]; then
    exit 0
...
load_nemo()
{
  BOOTKERNEL=$1
  menu_fadeout
  TMP_COMMAND_LINE=$(echo "$O_COMMAND_LINE" | sed -e "s/root\=\/dev\/mmcblk0p2/root\=\/dev\/mmcblk0p4/")
  F_COMMAND_LINE="\"$TMP_COMMAND_LINE\""
  eval "kexec -l --type=zImage --command-line=$F_COMMAND_LINE $BOOTKERNEL"
  if [ $? -eq 0 ]; then
    exit 0
